return {
    ...
    config = function()
        ...
        vim.keymap.set('n', '<leader>fb', builtin.buffers, { desc = 'Find [B]uffer'})
        vim.keymap.set('n', '<leader>fw', builtin.grep_string, { desc = 'Search Focus [W]ord'})
        vim.keymap.set('n', '<leader>fr', builtin.oldfiles, { desc = 'Find [R]ecent Files'})
        vim.keymap.set('n', '<leader>fs', builtin.treesitter, { desc = 'Find [S]ymbols'})
    end
}
